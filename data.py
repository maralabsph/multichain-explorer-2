...
#                    body += '<td>'+offchain_html+'</td>'
    ...
    def general_data_html(self, chain, data, txid, vout):
        ...
        if printdata:
            # Check if mydata is a string and looks like a hex string.
            # A simple check: contains only hex characters (0-9, a-f).
            is_hex = isinstance(mydata, str) and len(mydata) > 0 and not re.search(r'[^0-9a-fA-F]', mydata)

            if is_hex:
                # If it's a hex string, create the span and button for conversion.
                escaped_hex = escape(mydata, quote=True)
                result = (
                    f'<span data-original-hex="{escaped_hex}" data-showing-hex="true">{escaped_hex}</span>'
                    f'<button class="btn btn-primary btn-xs" onclick="toggleHex(this)" style="margin-left: 10px;">Show Text</button>'
                )
            else:
                # Otherwise, use the original logic for displaying other data (like plain text).
                result = str(mydata)
                size = len(result)
                result = escape(result[:MAX_SHOWN_DATA], quote=True)
                if size > MAX_SHOWN_DATA:
                    result += '<span class="ellipses" data-toggle="popover" data-trigger="hover" data-content=" ' + escape(str(mydata), quote=True) + ' ">...</span>'
